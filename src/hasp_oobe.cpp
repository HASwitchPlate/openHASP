...
void gotoPage1_cb(lv_event_t* e)
{
...
static void kb_event_cb(lv_event_t* e)
{
    ...
    if(event == LV_EVENT_READY) { // LV_EVENT_APPLY
        ...
        if(event == LV_EVENT_RELEASED) lv_keyboard_def_event_cb(e);
...
static void oobeSetupQR(const char* ssid, const char* pass)
{
    ...
    oobepage[0]     = lv_obj_create(NULL);
    char buffer[128];
    lv_obj_t* container = lv_cont_create(oobepage[0]);
    lv_obj_set_pos(container, 5, 5);
    // lv_obj_set_style_bg_opa(container, LV_ARC_PART_BG, LV_STATE_DEFAULT, 0);
    // lv_obj_set_style_border_opa(container, LV_ARC_PART_BG, LV_STATE_DEFAULT, 0);
    ...
    lv_obj_t* qrlabel = lv_label_create(oobepage[0]);
    ...
    lv_obj_t* password = lv_label_create(panel);
...
static void oobeSetupSsid(void)
{
    ...
    oobepage[1] = lv_obj_create(NULL);
    char buffer[32];

    /* Create the password box */
    pwd_ta = lv_textarea_create(oobepage[1]);
    lv_obj_set_style_text_font(pwd_ta, defaultfont, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    udata.id = 20;
    lv_obj_set_user_data(pwd_ta, (lv_obj_user_data_t*)&udata);
    ...
    lv_obj_t* pwd_icon = lv_btn_create(oobepage[1]);
    ...
    lv_obj_set_style_value_str(pwd_icon, LV_PART_MAIN | LV_STATE_DEFAULT, LV_SYMBOL_EYE_CLOSE);
    lv_obj_set_style_radius(pwd_icon, lv_obj_get_style_radius(pwd_ta, LV_TEXTAREA_PART_BG),
    ...
    lv_obj_set_style_text_font(oneline_ta, defaultfont, LV_OBJ_PART_MAIN | LV_STATE_DEFAULT);

    lv_textarea_set_password_mode(oneline_ta, false);
    udata.id = 10;
    lv_obj_set_user_data(oneline_ta, (lv_obj_user_data_t*)&udata);
    // lv_obj_align(oneline_ta, pwd_ta, LV_ALIGN_OUT_TOP_MID, 0, topmargin);

    /* Create a label and position it above the text box */
    lv_obj_t* pwd_label = lv_label_create(oobepage[1]);
    ...
    /* Create a label and position it above the text box */
    lv_obj_t* oneline_label = lv_label_create(oobepage[1]);
...
static void oobe_calibrate_cb(lv_event_t* e)
{
    lv_event_code_t event = lv_event_get_code(e);
