// created by Jean-Marc Zingg to be the GxCTRL_ILI9325C class for the GxTFT library
...
#include "GxCTRL_ILI9325C.h"

uint32_t GxCTRL_ILI9325C::readID()
{
  return readRegister(0x0, 0, 2);
}

uint32_t GxCTRL_ILI9325C::readRegister(uint8_t nr, uint8_t index, uint8_t bytes)
{
  uint32_t rv = 0;
  bytes = min(bytes, 4);
  IO.startTransaction();
...
uint16_t GxCTRL_ILI9325C::readPixel(uint16_t x, uint16_t y)
{
  uint16_t rv;
  readRect(x, y, 1, 1, &rv);
  return rv;
...
void GxCTRL_ILI9325C::readRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t* data)
{
  uint16_t xe = x + w - 1;
  uint16_t ye = y + h - 1;
...
void GxCTRL_ILI9325C::setWindowAddress(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1)
{
  switch (_rotation)
  {
    case 0:
...
void GxCTRL_ILI9325C::setRotation(uint8_t r)
{
  _rotation = r & 3;
  IO.startTransaction();
  switch (_rotation)
...
void GxCTRL_ILI9325C::LCD_Write_COM_DATA(uint8_t com, uint16_t data)
{
  IO.writeCommand(com);
  IO.writeData(data >> 8);
  IO.writeData(data);
